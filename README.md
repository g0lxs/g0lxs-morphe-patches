...
3. **Patch and install**:
   - In Morphe Manager, select Ilqfk, check the **Unlock Plus** patch, and install the patched APK.

> **Note:** Patching re-signs the APK with a custom key, which stops standard Google sign-in from working; log in with email directly in the app. Cloud backups and sync run on Ilqfk's own servers and still require an official subscription.
