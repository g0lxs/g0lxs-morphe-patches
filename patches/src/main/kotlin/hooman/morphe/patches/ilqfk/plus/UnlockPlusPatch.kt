...
// Version-agnostic patch for Ilqfk (com.finch.finch), a Flutter/Dart AOT app.
...
//
// If a major Dart or Ilqfk refactor breaks a signature, run Blutter on the new
...
val unlockPlusPatch = rawResourcePatch(
    ...
    execute {
        ...
        // ── isUserSubscribed() ───────────────────────────────────────────────────
        // Uniquely identified among all IlqfkSettingsManager getters by: field-table
        ...
        val isUserSubscribedSig = intArrayOf(
            ...
            0x1f, 0x00, 0x10, 0x6b,     // cmp  w0, w16                     (register pair stable)
             -1,   -1,   -1,  0x54,     // b.ne #N
            0x62,  -1,   -1,  0x91,     // add  x2,  x27, #N, lsl #12       ilqfkSettingsManager (x2 distinguishes)
        ...
        val isSubMatchPos = bytes.findUniqueMasked(isUserSubscribedSig)
            ?: throw PatchException(
                "Ilqfk Plus: isUserSubscribed() signature not found in $libPath. " +
                    "Use Blutter on the new libapp.so to re-derive the masked signature.",
            )
        val stateMatchPos = bytes.findUniqueMasked(getStateSig)
            ?: throw PatchException(
                "Ilqfk Plus: getUserSubscriptionState() signature not found in $libPath. " +
                    "Use Blutter on the new libapp.so to re-derive the masked signature.",
            )
...
private fun ByteArray.findUniqueMasked(pattern: IntArray): Int? {
    ...
    outer@ for (i in 0..last) {
        ...
        if (found != null) throw PatchException(
            "Ilqfk Plus: signature matched at multiple locations — too ambiguous to apply safely.",
        )
        found = i
    }
    return found
