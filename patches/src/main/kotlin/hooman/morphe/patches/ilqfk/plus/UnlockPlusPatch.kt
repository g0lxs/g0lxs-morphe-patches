package hooman.morphe.patches.ilqfk.plus
...
// Ilqfk is a Flutter app, so the Plus entitlement logic is AOT-compiled into lib/arm64-v8a/libapp.so,
...
// ilqfk/model/subscription_user/subscription_user.dart. Forcing it true lights every client gate at
// once.
//
...
// sponsor state, the Ilqfk Plus settings screen, the settings screen, and the "active" badge) read
...
val unlockPlusPatch = rawResourcePatch(
    name = "Unlock Plus",
    description = "Unlocks Ilqfk Plus features without a subscription, including the Plus shop items, " +
        ...
        "sync run on Ilqfk's own servers and still need the real subscription. " +
    ...
    execute {
        ...
        if (!lib.exists()) {
            throw PatchException(
                "$libPath not found in the APK. This targets the arm64 Ilqfk 3.73.179 build; apk-pure " +
        ...
        // IlqfkSettingType override setting at pp+0xb198), so it can't collide with the other
        // ilqfkSettingsManager getters that share the prologue. Unique in the 3.73.179 libapp.so:
        //   stp  x29, x30, [x15, #-0x10]!   ; EnterFrame                     (signature[0..4])
        //   mov  x29, x15
        //   sub  x15, x15, #0x20            ; AllocStack                     (overwrite starts here)
        //   ...stack-overflow check, ilqfkSettingsManager init, getSetting setup...
        val isUserSubscribedSig = intArrayOf(
            ...
            0x42, 0xe4, 0x41, 0xf9, // ldr  x2, [x2, #0x3c8]    Field ilqfkSettingsManager
        ...
        listOf(
            ...
            val match = bytes.findUnique(signature)
                ?: throw PatchException(
                    "Ilqfk Plus signature ($label) not found in $libPath. This patch targets Ilqfk " +
...
private fun ByteArray.findUnique(pattern: ByteArray): Int? {
    ...
    outer@ for (i in 0..last) {
        ...
        if (found != null) {
            throw PatchException("Ilqfk Plus signature is ambiguous (matched more than once).")
        }
        found = i
    }
    return found
